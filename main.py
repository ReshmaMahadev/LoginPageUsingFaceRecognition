...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'rmahadevappa@arigs.com' ")
...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        if Id!=Id:
            ...
            mess._show(title='Employee Registration', message=' Employee ID already exist')
    else:
...
def TrackImages():
    check_haarcascadefile()
    assure_path_exists("EmployeeLoginDetails/")
    assure_path_exists("EmployeeDetails/")
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                bb = str(aa)
                bb = bb[2:-2]
                EmployeeLoginDetails = [str(ID), '', bb, '', str(date), '', str(timeStamp)]

            else:
                Id = 'Unknown'
                bb = str(Id)
            cv2.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 255), 2)
        cv2.imshow('Employee Login Details', im)
        if (cv2.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("EmployeeLoginDetails\EmployeeLoginDetails_" + date + ".csv")
    if exists:
        with open("EmployeeLoginDetails\EmployeeLoginDetails_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            writer.writerow(EmployeeLoginDetails)
        csvFile1.close()
    else:
        with open("EmployeeLoginDetails\EmployeeLoginDetails_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            ...
            writer.writerow(EmployeeLoginDetails)
        csvFile1.close()
    with open("EmployeeLoginDetails\EmployeeLoginDetails_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
message3 = tk.Label(window, text="Face Recognition Based Login Monitoring System" ,fg="white",bg="#2d420a" ,width=55 ,height=1,font=('comic', 29, ' bold '))
